...
class _IngredientDetailsState extends State<IngredientDetails> {
  ...
  void initState() {
    ...
    descriptionCheck = widget.ingredient.ingredientDescription == "" ? false : true;
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: <Widget>[
              ...
              ReemKufi_Green_Bold_Center(textValue: widget.ingredient.ingredientName.trim() , size: displayWidth(context) * 0.08 ),
              SizedBox(height: displayHeight(context) * 0.01),
              if(ratingCheck)...{
                ReemKufi_Green_Bold_Left(textValue: 'Rating: ' , size: displayHeight(context) * 0.035 ),
                Raleway_Green_Bold_Left(textValue: widget.ingredient.ingredientRating.trim() , size: displayHeight(context) * 0.02 ),
                SizedBox(height: displayHeight(context) * 0.01),
              },
              if(categoriesCheck)...{
                ReemKufi_Green_Bold_Left(textValue: 'Categories: ', size: displayHeight(context) * 0.035),
                Raleway_Green_Bold_Left(textValue: widget.ingredient.ingredientCategory.trim(), size: displayHeight(context) * 0.02),
                SizedBox(height: displayHeight(context) * 0.01),
              },
              if(descriptionCheck)...{
                ReemKufi_Green_Bold_Left(textValue: 'Descripion: ', size: displayHeight(context) * 0.035),
                Raleway_Green_Bold_Left(textValue:   widget.ingredient.ingredientDescription.trim(), size: displayHeight(context) * 0.02),
                SizedBox(height: displayHeight(context) * 0.01),
              },
              SizedBox(height: displayHeight(context) * 0.099),
              Padding(
                padding: EdgeInsets.fromLTRB(displayWidth(context)*0.02, 0, displayWidth(context)*0.02, 0),
                child: Container(
                  height: displayHeight(context)*0.25,
                  decoration: BoxDecoration(
                    borderRadius: BorderRadius.circular(20),
